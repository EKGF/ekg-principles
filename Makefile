...
EKG_MANIFESTO_VERSION := $(shell cat ekg-manifesto/VERSION)
PDF_FILE_NAME_SUFFIX := $(EKG_MANIFESTO_VERSION)-$(USER)-$(CURRENT_BRANCH)
EKG_MANIFESTO_EDITORS_VERSION := $(subst .,-,$(subst _,-,ekg-manifesto/ekgf-ekg-manifesto-editors-version-$(PDF_FILE_NAME_SUFFIX))).pdf
EKG_MANIFESTO_RELEASE_VERSION := $(subst .,-,$(subst _,-,ekg-manifesto/ekgf-ekg-manifesto-$(PDF_FILE_NAME_SUFFIX))).pdf
MKDOCS = $(shell asdf where python)/bin/mkdocs

.PHONY: all
all: $(EKG_MANIFESTO_EDITORS_VERSION) $(EKG_MANIFESTO_RELEASE_VERSION)

.PHONY: info
info:
	@echo "VERSION: ${EKG_MANIFESTO_VERSION}"
	@echo "Suffix: ${PDF_FILE_NAME_SUFFIX}"
	@echo "Release Version: ${EKG_MANIFESTO_RELEASE_VERSION}"
	@echo "Editors Version: ${EKG_MANIFESTO_EDITORS_VERSION}"
...
open-editors-version-macos: $(EKG_MANIFESTO_EDITORS_VERSION)
	skim $(EKG_MANIFESTO_EDITORS_VERSION)
...
open-release-version-macos: $(EKG_MANIFESTO_RELEASE_VERSION)
	skim $(EKG_MANIFESTO_RELEASE_VERSION)
...
release-version:
	latex_document_mode=release-version latex_document_main=ekg-manifesto latex_customer_code=ekgf latexmk -gg -pvc

$(EKG_MANIFESTO_RELEASE_VERSION): release-version

.PHONY: editors-version
editors-version:
	latex_document_mode=editors-version latex_document_main=ekg-manifesto latex_customer_code=ekgf latexmk -gg -pvc

$(EKG_MANIFESTO_EDITORS_VERSION): editors-version

.PHONY: clean
clean:
	@echo "Removing all generated files"
	@cd ekg-manifesto && rm -f *.acn *.acr *.alg *.aux *.bbl *.bcf *.blg *.fdb* *.fls *.gl? *.idx *.ilg *.ind *.ist *.log *.odn *.ol? *.pdf *.run.xml *.sync* *.tdn *.tl? *.toc
